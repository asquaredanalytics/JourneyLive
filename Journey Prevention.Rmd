...
Comp_DF$PERIOD_RISK <- Comp_DF$ANY_RISK
...
testdf <- testdf[, -c(3:30,55:110)]

subset_data <- testdf %>%
  group_by(USERID) %>%
  filter(any(ANY_RISK == 0)) %>%
  ungroup()
...
testdf1 <- testdf %>%
  arrange(USERID, EVENTDATE) %>%              # make sure rows are in time order within person
  group_by(USERID) %>%
  summarise(
    had_zero = any(ANY_RISK == 0),
    had_zero_then_one = any(ANY_RISK == 1 & cummax(ANY_RISK == 0 & row_number() < which(ANY_RISK == 1)[1]))
  )
...
transitioned_users <- testdf %>%
  arrange(USERID, EVENTDATE) %>%
  group_by(USERID) %>%
  mutate(ever_zero_so_far = cummax(ANY_RISK == 0)) %>%
  filter(ANY_RISK == 1 & lag(ever_zero_so_far, default = FALSE)) %>%
  ungroup() %>%
  distinct(USERID) %>%
  pull(USERID)
