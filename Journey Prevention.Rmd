...
# Random intercept model: allows each person to have their own baseline stress level
model1 <- lmer(FEELINGRATING ~ TOTAL_JOURNEY_USE_D + (1 | USERID), data = Daily_DF_Risk_Modeling)

summary(model1)

# Random intercept + random slope model: allows the effect of TOTAL_USE_D 
# to vary by person as well (recommended if you expect individual differences 
# in how TOTAL_USE_D relates to FEELINGRATING over time)


model2 <- lmer(FEELINGRATING ~ TOTAL_JOURNEY_USE_D_z + (1 + TOTAL_JOURNEY_USE_D_z | USERID), 
               data = Daily_DF_Risk_Modeling)
...
Daily_DF_Risk_Modeling$DATE_NUMERIC <- as.numeric(Daily_DF_Risk_Modeling$EVENTDATE - min(Daily_DF_Risk_Modeling$EVENTDATE))

model3 <- lmer(FEELINGRATING ~ TOTAL_JOURNEY_USE_D_z + DATE_NUMERIC + (1 + TOTAL_JOURNEY_USE_D_z | USERID), 
               data = Daily_DF_Risk_Modeling)

summary(model3)

# Compare the two models to see if adding the random slope improves fit
...
tvem_model_M <- tvem(
  formula = DAYSEMOTIONALLYCHALLENGING ~ TOTAL_JOURNEY_USE_M_z,
  data = Monthly_DF_Risk_Modeling,
  id = USERID,
  time = DATE_NUMERIC,
  family = gaussian() # Use binomial() for binary outcomes
)
